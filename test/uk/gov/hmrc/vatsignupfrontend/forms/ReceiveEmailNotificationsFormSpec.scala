...
class ReceiveEmailNotificationsFormSpec extends UnitSpec {

  val error = "error.receive_email_notification"

  "YesNoForm" should {
    "successfully parse a Yes" in {
      val res = receiveEmailNotificationForm.bind(Map(yesNo -> option_yes))
      res.value should contain(Yes)
    }

    "successfully parse a No" in {
      val res = receiveEmailNotificationForm.bind(Map(yesNo -> option_no))
      res.value should contain(No)
    }

    "fail when nothing has been entered" in {
      val res = receiveEmailNotificationForm.bind(Map.empty[String, String])
