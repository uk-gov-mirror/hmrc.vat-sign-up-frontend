...
class SoftwareReadyFormSpec extends UnitSpec {

  val error = "error.software_ready"

  "YesNoForm" should {
    "successfully parse a Yes" in {
      val res = softwareReadyForm.bind(Map(yesNo -> option_yes))
      res.value should contain(Yes)
    }

    "successfully parse a No" in {
      val res = softwareReadyForm.bind(Map(yesNo -> option_no))
      res.value should contain(No)
    }

    "fail when nothing has been entered" in {
      val res = softwareReadyForm.bind(Map.empty[String, String])
