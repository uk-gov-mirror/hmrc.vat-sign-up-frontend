...
import uk.gov.hmrc.vatsignupfrontend.assets.MessageLookup.{PrincipalConfirmCompanyNumber => messages}
...
class ConfirmCompanyNumberSpec extends ViewSpec {
  ...
  lazy val page = uk.gov.hmrc.vatsignupfrontend.views.html.principal.confirm_company_number(
  ...
  "The Confirm Company Number view" should {
    ...
    testPage.shouldHaveALink(
      id = "changeLink",
      text = messages.link,
      href = uk.gov.hmrc.vatsignupfrontend.controllers.principal.routes.CaptureCompanyNumberController.show().url
    )
  }
