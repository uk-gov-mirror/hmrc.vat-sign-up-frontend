...
import uk.gov.hmrc.vatsignupfrontend.assets.MessageLookup.{ConfirmClient => messages}
...
class CheckYourClientDetailsViewSpec extends ViewSpec {
  ...
  def page(): Html = uk.gov.hmrc.vatsignupfrontend.views.html.agent.check_your_client_details(
  ...
  "display the correct info for firstName" in {
    ...
    val expectedEditLink = uk.gov.hmrc.vatsignupfrontend.controllers.agent.routes.CaptureClientDetailsController.show().url
