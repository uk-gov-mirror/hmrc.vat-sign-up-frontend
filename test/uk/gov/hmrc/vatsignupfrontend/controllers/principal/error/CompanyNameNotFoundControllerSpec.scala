...
class CompanyNameNotFoundControllerSpec extends UnitSpec with GuiceOneAppPerSuite with MockVatControllerComponents {
  ...
  lazy val testGetRequest: FakeRequest[AnyContentAsEmpty.type] = FakeRequest("GET", "/error/company-name-not-found")

  lazy val testPostRequest: FakeRequest[AnyContentAsEmpty.type] =
    FakeRequest("POST", "/error/company-name-not-found")
  ...
  "Calling the submit action of the Company Name not found controller" should {
    "redirect to capture your company number page" in {
      ...
      redirectLocation(result) should contain(principalRoutes.CaptureCompanyNumberController.show().url)
    }
  }
