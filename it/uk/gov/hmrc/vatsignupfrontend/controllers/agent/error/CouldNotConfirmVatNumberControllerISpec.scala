...
class CouldNotConfirmVatNumberControllerISpec extends ComponentSpecBase with CustomMatchers {
  ...
  "GET /client/error/could-not-confirm-vat-number" should {
    "return an OK" in {
      stubAuth(OK, successfulAuthResponse(agentEnrolment))

      val res = get("/client/error/could-not-confirm-vat-number")
  ...
  "POST /client/error/could-not-confirm-vat-number" should {
    "redirect to the capture vat number page" in {
      stubAuth(OK, successfulAuthResponse(agentEnrolment))

      val res = post("/client/error/could-not-confirm-vat-number")()
