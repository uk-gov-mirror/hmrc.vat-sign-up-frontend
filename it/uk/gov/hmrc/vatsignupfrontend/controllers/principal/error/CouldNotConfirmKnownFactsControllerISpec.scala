...
class CouldNotConfirmKnownFactsControllerISpec extends ComponentSpecBase with CustomMatchers {
  ...
  "GET /error/could-not-confirm-partnership" should {
    "return an OK" in {

      stubAuth(OK, successfulAuthResponse())

      val res = get("/error/could-not-confirm-partnership")

      res should have(httpStatus(OK))
    }
  }

  "POST /error/could-not-confirm-partnership" should {
    "redirect to the capture partnership utr number page" in {

      stubAuth(OK, successfulAuthResponse())

      val res = post("/error/could-not-confirm-partnership")()

      res should have(
        httpStatus(SEE_OTHER),
        redirectUri(partnershipsRoutes.CapturePartnershipUtrController.show().url)
      )
    }
  }
