...
import uk.gov.hmrc.vatsubscriptionfrontend.views.html.no_agent_client_relationship
...
class NoAgentClientRelationshipController @Inject()(val controllerComponents: ControllerComponents)
  ...
  val show: Action[AnyContent] = Action.async { implicit request =>
    authorised() {
      ...
      Future.successful(
        Ok(no_agent_client_relationship(routes.ConfirmationController.submit()))
      )
    }
  }
