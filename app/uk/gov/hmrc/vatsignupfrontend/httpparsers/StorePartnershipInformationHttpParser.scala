...
import play.api.http.Status.{NO_CONTENT, FORBIDDEN, PRECONDITION_FAILED}
import uk.gov.hmrc.http.{HttpReads, HttpResponse}

object StorePartnershipInformationHttpParser {
  type StorePartnershipInformationResponse = Either[StorePartnershipInformationFailure, StorePartnershipInformationSuccess.type]

  implicit object StorePartnershipInformationHttpReads extends HttpReads[StorePartnershipInformationResponse] {
    override def read(method: String, url: String, response: HttpResponse): StorePartnershipInformationResponse =
      response.status match {
        ...
        case PRECONDITION_FAILED => Left(PartnershipUtrNotFound)
  ...
  case object PartnershipUtrNotFound extends StorePartnershipInformationFailure
