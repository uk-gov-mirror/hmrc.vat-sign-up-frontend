...
@Singleton
class CaptureEmailPasscodeController @Inject()(emailVerificationService: EmailVerificationService,
  ...
  def show(): Action[AnyContent] = Action.async { implicit request =>
    authorised() {
      request.session.get(SessionKeys.emailKey) match {
        case Some(email) =>
          emailVerificationService.requestEmailVerificationPasscode(email, request.lang.code) map {
            case AlreadyVerifiedEmailAddress =>
              Redirect(principalRoutes.EmailVerifiedController.show())
      ...
      (request.session.get(SessionKeys.vatNumberKey), request.session.get(SessionKeys.emailKey)) match {
        case (Some(vatNumber), Some(email)) =>
          EmailPasscodeForm().bindFromRequest() fold (
            ...
            passcode =>
              emailVerificationService.verifyEmailPasscode(email, passcode) flatMap {
                ...
                case EmailAlreadyVerified | EmailVerifiedSuccessfully =>
                  storeEmailAddressService.storeEmailAddress(vatNumber, email) map {
                    case Right(StoreEmailAddressSuccess(_)) =>
                      Redirect(principalRoutes.EmailVerifiedController.show())
                    case _ =>
                      throw new InternalServerException("[CaptureEmailPasscodeController][submit] Failed to store email address")
                  }
              }
          )
