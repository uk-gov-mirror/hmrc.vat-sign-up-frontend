...
class CheckYourAnswersPartnershipsController @Inject()(val controllerComponents: ControllerComponents,
  ...
  def show: Action[AnyContent] = Action.async { implicit request =>
    authorised() {
      ...
      optBusinessEntityType match {
        ...
        case None => Future.successful(Redirect(principalRoutes.CaptureBusinessEntityController.show()))
      }
    }
  }
  ...
  private def showGeneralPartnershipAnswers()(implicit request: Request[_]): Future[Result] = {
    ...
    (isEnabled(JointVenturePropertyJourney), optJointVentureProperty) match {
      case (true, None) => Future.successful(Redirect(routes.JointVentureOrPropertyController.show()))
      case (true, Some(Yes)) =>
  ...
  private def utrPostcodePredicate(f: (String, PostCode) => Future[Result])(implicit request: Request[_]): Future[Result] = {
    ...
    (optPartnershipUtr, optPartnershipPostCode) match {
      ...
      case (_, None) => Future.successful(Redirect(routes.PrincipalPlacePostCodeController.show()))
    }
  }
  ...
  private def crnAndEntityTypePredicate(f: (String, PartnershipEntityType) => Future[Result])(implicit request: Request[_]): Future[Result] = {
    ...
    (optPartnershipCrn, optPartnershipType) match {
      ...
      case (_, _) => Future.successful(Redirect(routes.CapturePartnershipCompanyNumberController.show()))
    }
  }
  ...
  def submit: Action[AnyContent] = Action.async { implicit request =>
    authorised() {
      ...
      (optVatNumber, optBusinessEntityType) match {
        ...
        case (None, _) => Future.successful(Redirect(principalRoutes.ResolveVatNumberController.resolve()))
        case (_, None) => Future.successful(Redirect(principalRoutes.CaptureBusinessEntityController.show()))
      }
    }
  }
