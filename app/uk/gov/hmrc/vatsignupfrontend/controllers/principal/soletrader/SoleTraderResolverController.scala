    ...
    implicit request =>
      authorised()(Retrievals.allEnrolments and Retrievals.nino) {
        case enrolments ~ optNino =>
          (enrolments.selfAssessmentUniqueTaxReferenceNumber, optNino) match {
            case (None, None) => Future.successful(Redirect(CaptureYourDetailsController.show()))
            case (Some(utr), _) =>
              citizenDetailsService.getCitizenDetailsBySautr(utr) map {
                case Right(CitizenDetailsRetrievalSuccess(detailsModel)) =>
                  Redirect(ConfirmYourRetrievedUserDetailsController.show())
