...
import uk.gov.hmrc.vatsignupfrontend.controllers.principal
...
class SessionTimeoutController @Inject()(val appConfig: AppConfig,
  ...
  val timeout: Action[AnyContent] = Action.async { implicit request =>
    Future.successful(toGGLogin(principal.routes.ResolveVatNumberController.resolve().url).withNewSession)
  }
