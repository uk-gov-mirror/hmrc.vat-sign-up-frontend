...
@Singleton
class GotSoftwareController @Inject()(val controllerComponents: ControllerComponents) extends FrontendController with I18nSupport {

  final val messagesApi: MessagesApi = controllerComponents.messagesApi
  implicit final val appConfig: AppConfig = controllerComponents.appConfig

  def show: Action[AnyContent] = Action { implicit request =>
    Ok(eligibility.got_software(principalRoutes.ResolveVatNumberController.resolve()))
  }
