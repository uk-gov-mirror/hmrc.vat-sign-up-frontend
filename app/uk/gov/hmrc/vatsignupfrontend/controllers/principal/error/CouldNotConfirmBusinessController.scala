...
@Singleton
class CouldNotConfirmBusinessController @Inject()(implicit ec: ExecutionContext,
    ...
    authorised() {
      Future.successful(
        optBusinessEntity match {
          case Some(_: BusinessEntity) => Redirect(principalRoutes.CaptureBusinessEntityController.show().url).removingFromSession(
            businessEntityKey,
            companyNumberKey,
            companyUtrKey,
            ninoKey,
          ...
          case _ => Redirect(principalRoutes.CaptureVatNumberController.show().url)
        }
      )
    }
  }
