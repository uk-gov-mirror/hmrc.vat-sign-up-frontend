...
@Singleton
class ConfirmSocietyController @Inject()(val controllerComponents: ControllerComponents,
  ...
  val show: Action[AnyContent] = Action.async { implicit request =>
    authorised() {
      val optSocietyName = request.session.get(societyNameKey).filter(_.nonEmpty)
      Future.successful(
        optSocietyName match {
          case Some(societyName) =>
            val changeLink = routes.CaptureSocietyCompanyNumberController.show().url
            Ok(confirm_registered_society(
              societyName = societyName,
              postAction = routes.ConfirmSocietyController.submit(),
              changeLink = changeLink
            ))
          case _ =>
            Redirect(routes.CaptureSocietyCompanyNumberController.show())
        }
      )
    }
  }
    ...
    authorised()(Retrievals.allEnrolments) {
      enrolments => {
        ...
        (optVatNumber, optCompanyNumber) match {
          ...
          case _ =>
            Future.successful(Redirect(routes.CaptureSocietyCompanyNumberController.show()))
        }
      }
    }
  }
